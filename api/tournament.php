...
function createGymLeaderBattleState($pdo, $playerId, $playerTeam, $route) {
    ...
    // Format player team for battle (apply stat bonuses)
    $formattedPlayerTeam = array_map(function($p) {
        ...
        return [
            ...
            'is_fainted' => false,
            'bonus_hp' => $bonusHp,
            'bonus_attack' => $bonusAtk,
            'bonus_speed' => $bonusSpd
        ];
    }, $playerTeam);
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'tournament.php') {
...
} // End of guard: only execute routing when called directly
